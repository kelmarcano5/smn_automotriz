package domain.repgen.smn_registro.smn_diagnostico_det_auto;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_servicio_rf_name");
			if (d != null)
			{
			name  = "b_service";
			label = "${lbl:b_service}";
		...
		d = rs.getString("did_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_diagnostico_cab_auto_id_name");
			if (d != null)
			{
			name  = "b_diagnostig_cab";
			label = "${lbl:b_diagnostig_cab}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("did_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_servicio_rf");
			if (d != null)
			{
			name  = "b_service";
			label = "${lbl:b_service}";
		...
		d = rs.getString("smn_diagnostico_cab_auto_id");
			if (d != null)
			{
			name  = "b_diagnostig_cab";
			label = "${lbl:b_diagnostig_cab}";
