package domain.repgen.smn_registro.smn_presupuesto;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
			...
			if (d != null)
			{
			name  = "b_pre_documento";
			label = "${lbl:b_pre_documento}";
		...
		d = rs.getString("pre_numero_control");
			if (d != null)
			{
			name  = "b_pre_num_control";
			label = "${lbl:b_pre_num_control}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("pre_fecha_registro");
			if (d != null)
			{
