package domain.repgen.smn_planes;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("pla_codigo_plan");
		if (d != null) 
		{
			name  = "pla_codigo_plan";
			label = "${lbl:b_code}";
		...
		//fila 1 - valor de busqueda
		d = StringUtil.formatDate(rs.getDate("pla_vigencia_desde"), "dd-MM-yyyy");
		if (d != null)
		{
			name  = "pla_vigencia_desde";
			label = "${lbl:b_from_date}";
		...
		if (d != null)
		{
			...
			d = StringUtil.formatDate(rs.getDate("pla_vigencia_hasta"), "dd-MM-yyyy");
			c = new PdfPCell( new Phrase( d , tblBodyFont ) );
			c.setHorizontalAlignment(Element.ALIGN_LEFT);
			datatable.addCell(c);
		}
