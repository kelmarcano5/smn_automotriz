package domain.repgen.smn_configuracion.smn_financiera.smn_sub_tipo_concepto;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("stc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_tipo_concepto_id_name");
			if (d != null)
			{
			name  = "b_type_concept";
			label = "${lbl:b_type_concept}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_concepto_id");
			if (d != null)
			{
			name  = "b_type_concept";
			label = "${lbl:b_type_concept}";
		...
		d = rs.getString("stc_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("stc_descripcion");
			if (d != null)
			{
			name  = "b_description";
