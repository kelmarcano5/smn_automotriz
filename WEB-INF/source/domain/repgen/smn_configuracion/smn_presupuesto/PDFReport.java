package domain.repgen.smn_presupuesto;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		name  = "pre_numero_presupuesto";
		label = "${lbl:b_budget_number}";
		...
		d = rs.getString("pre_numero_presupuesto");
		if (d != null) 
		{
			d = d.replaceAll("%", "");
		...
		//fila 1 - valor de busqueda
		d = rs.getString("pre_beneficiario");
		if (d != null)
		{
			name  = "pre_beneficiario";
			label = "${lbl:b_beneficiario}";
		...
		//fila 1 - valor de busqueda
		d = rs.getString("pre_vigencia_hasta");
		if (d != null)
		{
			name  = "pre_vigencia_hasta";
			label = "${lbl:b_fecha_vigencia}";
		...
		//fila 1 - valor de busqueda
		d = rs.getString("pre_monto_presupuesto");
		if (d != null)
		{
			name  = "pre_monto_presupuesto";
			label = "${lbl:b_monto_presupuesto}";
		...
		//fila 1 - valor de busqueda
				d = rs.getString("pre_estado_presupuesto");
				if (d != null)
				{
					name  = "pre_estado_presupuesto";
					label = "${lbl:b_budget_status}";
