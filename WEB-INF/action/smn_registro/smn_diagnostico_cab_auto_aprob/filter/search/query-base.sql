...
from
	...
	left outer join smn_base.smn_prestadores_servicios ps on ps.smn_prestadores_servicios_id = smn_automotriz.smn_diagnostico_cab_auto.smn_auxiliar_rf_prest
	left join smn_seguridad.s_user on smn_seguridad.s_user.userlogin = '${def:user}'
	left join smn_base.smn_usuarios on smn_base.smn_usuarios.smn_user_rf = smn_seguridad.s_user.user_id
	left join smn_automotriz.smn_rol on smn_automotriz.smn_rol.smn_usuario_id = smn_base.smn_usuarios.smn_usuarios_id
where
	smn_automotriz.smn_diagnostico_cab_auto.smn_diagnostico_cab_auto_id is not null --and smn_automotriz.smn_rol.rol_tipo<>'AP'
	${filter}
order by 
smn_automotriz.smn_diagnostico_cab_auto.dia_fecha_registro desc
