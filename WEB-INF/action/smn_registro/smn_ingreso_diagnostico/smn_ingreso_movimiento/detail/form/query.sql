select
	...
	(select smn_base.smn_componentes.cmp_codigo || ' - ' || smn_base.smn_componentes.cmp_descripcion from  smn_base.smn_componentes  where smn_base.smn_componentes.smn_componentes_id is not null  and smn_base.smn_componentes.smn_componentes_id=smn_automotriz.smn_ingreso_movimiento.smn_componente_rf  order by smn_base.smn_componentes.cmp_descripcion ) as smn_componente_rf_combo,
	(select smn_automotriz.smn_tipo_consulta.tcs_codigo || ' - ' || smn_automotriz.smn_tipo_consulta.tcs_nombre from  smn_automotriz.smn_tipo_consulta  where smn_automotriz.smn_tipo_consulta.smn_tipo_consulta_id is not null  and smn_automotriz.smn_tipo_consulta.smn_tipo_consulta_id=smn_automotriz.smn_ingreso_movimiento.smn_tipo_consulta_rf  order by smn_automotriz.smn_tipo_consulta.tcs_nombre ) as smn_tipo_consulta_rf_combo,
	...
	(select smn_automotriz.smn_motivos.mtv_codigo || ' - ' || smn_automotriz.smn_motivos.mtv_descripcion from  smn_automotriz.smn_motivos  where smn_automotriz.smn_motivos.smn_motivos_id is not null  and smn_automotriz.smn_motivos.smn_motivos_id=smn_automotriz.smn_ingreso_movimiento.smn_motivos_id  order by smn_automotriz.smn_motivos.mtv_descripcion ) as smn_motivos_id_combo,
	(select smn_automotriz.smn_sintoma.stm_codigo || ' - ' || smn_automotriz.smn_sintoma.stm_descripcion from  smn_automotriz.smn_sintoma  where smn_automotriz.smn_sintoma.smn_sintoma_id is not null  and smn_automotriz.smn_sintoma.smn_sintoma_id=smn_automotriz.smn_ingreso_movimiento.smn_sintoma_id  order by smn_automotriz.smn_sintoma.stm_descripcion ) as smn_sintoma_id_combo,
	case
		when smn_automotriz.smn_ingreso_movimiento.inm_estatus ='AC' then 'Activo'
		when smn_automotriz.smn_ingreso_movimiento.inm_estatus ='PR' then 'Procesado'
	end as inm_estatus_combo,
	case
	when smn_automotriz.smn_ingreso_movimiento.inm_tipo_orden='UR' then '${lbl:b_Urgente}'
	when smn_automotriz.smn_ingreso_movimiento.inm_tipo_orden='CO' then '${lbl:b_control}'
	when smn_automotriz.smn_ingreso_movimiento.inm_tipo_orden='SE' then '${lbl:b_segunda_opinion}'
