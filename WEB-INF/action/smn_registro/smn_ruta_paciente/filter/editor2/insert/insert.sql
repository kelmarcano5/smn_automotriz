...
(
	...
	${fld:smn_grupo_prestador_rf},
  	...
  	${fld:rta_estatus},
  	(select DISTINCT smn_automotriz.smn_ingreso_cola_virtual.smn_unidad_servicio_rf from smn_automotriz.smn_ingreso_cola_virtual
    LEFT OUTER JOIN smn_automotriz.smn_series on smn_automotriz.smn_series.sri_codigo= smn_automotriz.smn_ingreso_cola_virtual.smn_serie_id
    LEFT OUTER JOIN smn_base.smn_auxiliar on smn_base.smn_auxiliar.aux_codigo = ${fld:rta_numero_documento_identidad}
    where smn_automotriz.smn_ingreso_cola_virtual.smn_area_servicio_rf=${fld:smn_area_servicio} and smn_automotriz.smn_ingreso_cola_virtual.icv_numero_documento_identidad=${fld:rta_numero_documento_identidad}),
	${fld:smn_serie_id},
	(select sri_ultimo_numero+1 from smn_automotriz.smn_series where sri_codigo = ${fld:smn_serie_id}),
...
INSERT INTO smn_automotriz.smn_ruta_paciente
    ...
    where smn_automotriz.smn_relacion_unidad_paso.smn_area_servicio_rf=${fld:smn_area_servicio} and smn_automotriz.smn_series.sri_codigo=${fld:smn_serie_id} 
    ...
    and smn_automotriz.smn_relacion_unidad_paso.rup_unidad_administrativa='AD' and smn_automotriz.smn_relacion_unidad_paso.rup_secuencia=1 limit 1),
    ${fld:smn_serie_id},
    (select sri_ultimo_numero from smn_automotriz.smn_series where sri_codigo = ${fld:smn_serie_id}),
    '${def:time}',
    '${def:locale}',
    '${def:user}',
    '${def:date}',
