select
	...
	(select smn_base.smn_paso.pas_codigo || ' - ' ||  smn_base.smn_paso.pas_descripcion from  smn_base.smn_paso  where smn_base.smn_paso.smn_paso_id is not null  and smn_base.smn_paso.smn_paso_id=smn_automotriz.smn_relacion_unidad_paso.smn_paso_rf  order by smn_base.smn_paso.pas_descripcion ) as smn_paso_rf_combo,
	(select smn_automotriz.smn_series.sri_codigo || ' - ' ||  smn_automotriz.smn_series.sri_nombre from  smn_automotriz.smn_series  where smn_automotriz.smn_series.smn_series_id is not null  and smn_automotriz.smn_series.smn_series_id=smn_automotriz.smn_relacion_unidad_paso.smn_serie_id  order by smn_automotriz.smn_series.sri_nombre ) as smn_serie_id_combo,
	case
	...
	case
		when smn_automotriz.smn_relacion_unidad_paso.rup_unidad_administrativa='AD' then '${lbl:b_administrative}'
		when smn_automotriz.smn_relacion_unidad_paso.rup_unidad_administrativa='DS' then '${lbl:b_service}'
	end as rup_unidad_administrativa,
	smn_automotriz.smn_relacion_unidad_paso.rup_estatus,
	smn_automotriz.smn_relacion_unidad_paso.rup_vigencia,
	smn_automotriz.smn_relacion_unidad_paso.rup_fecha_registro
	
from
	smn_automotriz.smn_relacion_unidad_paso order by smn_automotriz.smn_relacion_unidad_paso.smn_serie_id asc
